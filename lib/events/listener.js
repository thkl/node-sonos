...
var eventModules = {
	"/MediaRenderer/RenderingControl/Event" : require("./RenderingControlEvent.js"),
	"/MediaRenderer/AVTransport/Event" : require("./TransportControlEvent.js"),
	"generic": require("./GenericEvent.js")
}
...
Listener.prototype._messageHandler = function (req, res) {
  if (req.method.toUpperCase() === 'NOTIFY' && req.url.toLowerCase() === '/notify') {
    ...
    this.parser.parseString(req.body.toString(), function (error, data) {
      ...
      if (thisService.endpoint in eventModules) {
	    event = new eventModules[thisService.endpoint];
	  } else {
	  	event = new eventModules['generic'];
	  }
	  
	  event.parse(thisService.data);
	  
      this.emit('serviceEvent', thisService.endpoint, req.headers.sid, event)
      res.end()
    }.bind(this))
  }
}
