...
var GROUP_ENDPOINT = '/ZoneGroupTopology/Control'
/**
 * Dependencies
 */
...
var Sonos = function Sonos (host, port, options) {
  ...
  if (!this.options.endpoints.group) this.options.endpoints.group = GROUP_ENDPOINT


}

Sonos.prototype.getEventListener = function() {
...
Sonos.prototype.getPositionInfo = function (callback) {
  debug('Sonos.getPositionInfo(' + ((callback) ? 'callback' : '') + ')')
  var self = this
  var action = '"urn:schemas-upnp-org:service:AVTransport:1#GetPositionInfo"'
  var body = '<u:GetPositionInfo xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><Channel>Master</Channel></u:GetPositionInfo>'
  var responseTag = 'u:GetPositionInfoResponse'
 ...
 */
Sonos.prototype.removeTrackFromQueue = function (trackNum, callback) {
  debug('Sonos.removeTrackFromQueue(' + ((callback) ? 'callback' : '') + ')')
  var self = this
  var action = '"urn:schemas-upnp-org:service:AVTransport:1#RemoveTrackFromQueue"'
  var body = '<u:RemoveTrackFromQueue xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><ObjectID>Q:0/'+trackNum+'</ObjectID></u:RemoveTrackFromQueue>'
 ...
 * @param  {Function} callback (err, volume)
 */
Sonos.prototype.getMediaInfo = function (callback) {
  debug('Sonos.getMediaInfo(' + ((callback) ? 'callback' : '') + ')')
 ...
 * @param  {Function} callback (err, muted)
 */
Sonos.prototype.enableSub = function (enable,callback) {
  debug('Sonos.enableSub(' + ((callback) ? 'callback' : '') + ')')
  var action = '"urn:schemas-upnp-org:service:RenderingControl:1#SetEQ"'
  var body = '<u:SetEQ xmlns:u="urn:schemas-upnp-org:service:RenderingControl:1"><InstanceID>0</InstanceID><EQType>SubEnable</EQType><DesiredValue>' + enable + '</DesiredValue></u:SetEQ>'
  var responseTag = 'u:SetEQResponse'
  return this.request(this.options.endpoints.rendering, action, body, responseTag, function (err, data) {
    if (err) return callback(err)
    callback(null, !!parseInt(data[0].CurrentMute[0], 10))
  })
...
/**
 * delegate Coordinator of  Group
 * @param  {Function} callback (err)
 */
Sonos.prototype.delegateGroupCoordinationTo = function (newCoordinator,callback) {
  debug('Sonos.DelegateGroupCoordinationTo(%s,%j)', newCoordinator, callback)
  var action, body
  action = '"urn:schemas-upnp-org:service:AVTransport:1#DelegateGroupCoordinationTo"'
  body = '<u:DelegateGroupCoordinationTo xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><NewCoordinator>' + newCoordinator + '</NewCoordinator><RejoinGroup>false</RejoinGroup></u:DelegateGroupCoordinationTo>'
  return this.request(this.options.endpoints.transport, action, body, 'u:DelegateGroupCoordinationToResponse', function (err, data) {
    if (err) return callback(err)
    if (data[0].$['xmlns:u'] === 'urn:schemas-upnp-org:service:AVTransport:1') {
      return callback(null, true)
    } else {
...
/**
 * Add Player to an existing group
 * @param  {Function} callback (err, stopped)
 */
Sonos.prototype.addPlayerToGroup = function (newMember,callback) {
  debug('Sonos.addPlayerToGroup(%s,%j)', newMember, callback)
  var action, body
  action = '"urn:schemas-upnp-org:service:GroupManagement:1#AddMember"'
  body = '<u:AddMember xmlns:u="urn:schemas-upnp-org:service:GroupManagement:1"><InstanceID>0</InstanceID><MemberID>'+newMember+'</MemberID></u:AddMember>'
  return this.request(this.options.endpoints.transport, action, body, 'u:AddMember', function (err, data) {
    if (err) return callback(err)
    if (data[0].$['xmlns:u'] === 'urn:schemas-upnp-org:service:GroupManagement:1') {
      return callback(null, true)
    } else {
      return callback(new Error({
        err: err,
...
/**
 * Adds a Spotify Playlist to the queue
 * @param  {String}   playlist      The spotify playlist uri
 
 * eq : 	spotify:user:spotify_germany:playlist:6OYlV1TOH1Awd8Hv6UOvyb


 * @param  {Function} callback (err, success)
 */
Sonos.prototype.addSpotifyPlaylist = function (playlisturi, callback) {
	

  var splaylisturi = playlisturi.replaceAll(":","%3a");
  var spuri = "0006206c" + splaylisturi;
  var sonosuri =  "x-rincon-cpcontainer:" + spuri ; 
  var spname = "Spotify";
  
  var metadata = "&lt;DIDL-Lite xmlns:dc=&quot;http://purl.org/dc/elements/1.1/&quot; xmlns:upnp=&quot;urn:schemas-upnp-org:metadata-1-0/upnp/&quot; xmlns:r=&quot;urn:schemas-rinconnetworks-com:metadata-1-0/&quot; xmlns=&quot;urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/&quot;&gt;&lt;item id=&quot;" + spuri + "&quot; parentID=&quot;00082064featured_pls&quot; restricted=&quot;true&quot;&gt;&lt;dc:title&gt;" + spname + "&lt;/dc:title&gt;&lt;upnp:class&gt;object.container.playlistContainer&lt;/upnp:class&gt;&lt;desc id=&quot;cdudn&quot; nameSpace=&quot;urn:schemas-rinconnetworks-com:metadata-1-0/&quot;&gt;SA_RINCON2311_X_#Svc2311-0-Token&lt;/desc&gt;&lt;/item&gt;&lt;/DIDL-Lite&gt;";
  var action = '"urn:schemas-upnp-org:service:AVTransport:1#AddURIToQueue"'
  var body = '<u:AddURIToQueue xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><EnqueuedURI>' + sonosuri + '</EnqueuedURI><EnqueuedURIMetaData>' + metadata + '</EnqueuedURIMetaData><DesiredFirstTrackNumberEnqueued>1</DesiredFirstTrackNumberEnqueued><EnqueueAsNext>1</EnqueueAsNext></u:AddURIToQueue>'
...
Sonos.prototype.getZoneGroupAttrs = function (callback) {
  debug('Sonos.getZoneGroupAttrs(%j, %j)', callback)
  var action = '"urn:schemas-upnp-org:service:ZoneGroupTopology:1#GetZoneGroupAttributes"'
  var body = '"<u:GetZoneGroupAttributes xmlns:u="urn:schemas-upnp-org:service:ZoneGroupTopology:1"><InstanceID>0</InstanceID></u:GetZoneGroupAttributes>"'
  this.request(this.options.endpoints.group, action, body, 'u:GetZoneGroupAttributesResponse', function (err, data) {
    if (err) return callback(err, data)
    var output = {}
    for (var d in data[0]) if (data[0].hasOwnProperty(d) && d !== '$') output[d] = data[0][d][0]
    callback(null, output)
...
Sonos.prototype.deviceDescription = function (callback) {
  debug('Sonos.deviceDescription(%j)', callback);
  request({
...
/**
 * RampToVolume
 * @param  {String}   Mode  _ SLEEP_TIMER_RAMP_TYPE , ALARM_RAMP_TYPE ,AUTOPLAY_RAMP_TYPE
 * @param  {String}   volume 0..100
 * @param  {Function} callback (err, data)
 * @return {[type]}
 */
Sonos.prototype.rampToVolume = function (ramptype,volume, callback) {
  debug('Sonos.rampToVolume(%j, %j, %j)', ramptype, volume, callback)
  var action = '"urn:schemas-upnp-org:service:RenderingControl:1#RampToVolume"'
  var body = '<u:RampToVolume xmlns:u="urn:schemas-upnp-org:service:RenderingControl:1"><InstanceID>0</InstanceID><Channel>Master</Channel><RampType>' + ramptype + '</RampType><DesiredVolume>' + volume + '</DesiredVolume><ResetVolumeAfter>false</ResetVolumeAfter><ProgramURI></ProgramURI></u:RampToVolume>'
  this.request(this.options.endpoints.rendering, action, body, '', function (err, data) {
    return callback(err, data)
  })
}
